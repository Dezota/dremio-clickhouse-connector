...
public class ClickHouseSchemaFetcherImpl extends JdbcSchemaFetcherImpl implements JdbcSchemaFetcher {
    public ClickHouseSchemaFetcherImpl(JdbcPluginConfig config) {
        super(config);
    }

    @Override
    public GetExternalQueryMetadataResponse getExternalQueryMetadata(GetExternalQueryMetadataRequest request) {
        return ClickHouseExternalQueryMetadataUtility.getBatchSchema(this.getDataSource(), (JdbcDremioSqlDialect) this.getConfig().getDialect(), request, this.getConfig());
    }
}
